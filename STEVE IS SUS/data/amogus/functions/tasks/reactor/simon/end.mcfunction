function amogus:tasks/end
tag @s remove task_simon
tag @s remove todo_l_reactor
tag @s remove simon_input
tag @s remove simon_display
tag @s remove simon_delay_input
scoreboard players set @s amogus_timer 0
scoreboard players set @s simon 0
scoreboard players set @s simon1 0
scoreboard players set @s simon2 0
scoreboard players set @s simon3 0
scoreboard players set @s simon4 0
scoreboard players set @s simon5 0